...
public class PieChart extends ViewGroup {
	...
	public PieChart(Context context, AttributeSet attrs) {
		...
		try {
			...
			mTextColor = a
					.getColor(R.styleable.PieChart_labelColor, 0xff000000);
			...
			mPointerRadius = a.getDimension(R.styleable.PieChart_pointerRadius,
					2.0f);
	 ...
	 * 
	 * @param textPos
	 *            TEXTPOS_LEFT to draw the text to the left of the graphic,
	 ...
	 * color of
	 * each segment to get the highlight color. A value of exactly one produces
	 * no
	 * highlight at all. Values greater than one produce highlights that are
	 * lighter
	 * than the base color, while values less than one produce highlights that
	 * are darker
	 * than the base color.
	 * 
	 ...
	 * view. This version
	 * is for internal use--the scrollIntoView option is always true for
	 * external callers.
	 * 
	 * @param currentItem
	 *            The index of the current item.
	 ...
	 * whose
	 * size is proportional to the item's value. As new items are added, the
	 * size of each
	...
	private void init() {
		...
		if (Build.VERSION.SDK_INT >= 11) {
			mAutoCenterAnimator = ObjectAnimator.ofInt(PieChart.this,
					"PieRotation", 0);
